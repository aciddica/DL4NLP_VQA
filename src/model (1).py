import mindspore
import word2vec
from archive_replaced.attention import Attention
...
class VQANet(mindspore.nn.Cell):
    def __init__(self, size_image = 224, size_word = 100, size_feature = 1024):
        ...
        self.attention = mindspore.nn.SequentialCell(
        [Attention(d = size_word, k = size_image),
            Attention(d = size_word, k = size_image)]
        )
    def construct(self, image, question):
        ...
        feature = question
        for att_layer in self.attention:
            feature = att_layer(image, question)
            question = feature
